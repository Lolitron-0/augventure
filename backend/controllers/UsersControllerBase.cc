...
void UsersControllerBase::getOne(
...
void UsersControllerBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Users::PrimaryKeyType&& id)
{
    ...
    try
    {
        if (isMasquerading())
        {
            if (!Users::validateMasqueradedJsonForUpdate(
...
void UsersControllerBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Users::PrimaryKeyType&& id)
{
...
void UsersControllerBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    auto jsonPtr = req->jsonObject();
    ...
    if (isMasquerading())
    {
        if (!Users::validateMasqueradedJsonForCreation(
                *jsonPtr, masqueradingVector(), err))
        {
            Json::Value ret;
            ret["error"] = err;
    ...
    try
    {
        Users object = (isMasquerading() ? Users(*jsonPtr, masqueradingVector())
                                         : Users(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr&)>>(
...
/*
void UsersControllerBase::update(const HttpRequestPtr &req,
...
UsersControllerBase::UsersControllerBase()
    : RestfulController({ "id", "email", "password", "username", "pfp_url",
                          "bio", "auth_code" })
{
    /**
    ...
    enableMasquerading({
        ...
        "pfp_url",  // the alias for the pfp_url column.
        "bio",      // the alias for the bio column.
        "auth_code" // the alias for the auth_code column.
    });
}

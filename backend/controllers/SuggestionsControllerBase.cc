...
void SuggestionsControllerBase::getOne(
...
void SuggestionsControllerBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Suggestions::PrimaryKeyType&& id)
{
    ...
    try
    {
        if (isMasquerading())
        {
            if (!Suggestions::validateMasqueradedJsonForUpdate(
...
void SuggestionsControllerBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Suggestions::PrimaryKeyType&& id)
{
...
void SuggestionsControllerBase::get(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    auto dbClientPtr = getDbClient();
...
void SuggestionsControllerBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    auto jsonPtr = req->jsonObject();
    ...
    if (isMasquerading())
    {
        if (!Suggestions::validateMasqueradedJsonForCreation(
                *jsonPtr, masqueradingVector(), err))
        {
            Json::Value ret;
            ret["error"] = err;
    ...
    {
        Suggestions object =
            (isMasquerading() ? Suggestions(*jsonPtr, masqueradingVector())
                              : Suggestions(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr&)>>(
...
SuggestionsControllerBase::SuggestionsControllerBase()
    : RestfulController(
          { "id", "author_id", "post_date", "sprint_id", "votes" })
{
    /**

...
void PostsControllerBase::getOne(
...
void PostsControllerBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Posts::PrimaryKeyType&& id)
{
    ...
    try
    {
        if (isMasquerading())
        {
            if (!Posts::validateMasqueradedJsonForUpdate(
...
void PostsControllerBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Posts::PrimaryKeyType&& id)
{
...
void PostsControllerBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    auto jsonPtr = req->jsonObject();
    ...
    if (isMasquerading())
    {
        if (!Posts::validateMasqueradedJsonForCreation(
                *jsonPtr, masqueradingVector(), err))
        {
            Json::Value ret;
            ret["error"] = err;
    ...
    try
    {
        Posts object = (isMasquerading() ? Posts(*jsonPtr, masqueradingVector())
                                         : Posts(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr&)>>(
...
PostsControllerBase::PostsControllerBase()
    : RestfulController({ "id", "text_content", "suggestion_id", "sprint_id" })
{
    /**

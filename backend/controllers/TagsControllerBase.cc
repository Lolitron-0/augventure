...
void TagsControllerBase::getOne(
...
void TagsControllerBase::updateOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Tags::PrimaryKeyType&& id)
{
    ...
    try
    {
        if (isMasquerading())
        {
            if (!Tags::validateMasqueradedJsonForUpdate(
...
void TagsControllerBase::deleteOne(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback,
    Tags::PrimaryKeyType&& id)
{
...
void TagsControllerBase::create(
    const HttpRequestPtr& req,
    std::function<void(const HttpResponsePtr&)>&& callback)
{
    auto jsonPtr = req->jsonObject();
    ...
    if (isMasquerading())
    {
        if (!Tags::validateMasqueradedJsonForCreation(
                *jsonPtr, masqueradingVector(), err))
        {
            Json::Value ret;
            ret["error"] = err;
    ...
    try
    {
        Tags object = (isMasquerading() ? Tags(*jsonPtr, masqueradingVector())
                                        : Tags(*jsonPtr));
        auto dbClientPtr = getDbClient();
        auto callbackPtr =
            std::make_shared<std::function<void(const HttpResponsePtr&)>>(
...
TagsControllerBase::TagsControllerBase()
    : RestfulController({ "id", "label", "counter" })
{
    /**

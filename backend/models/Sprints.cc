...
Sprints::Sprints(const Row &r, const ssize_t indexOffset) noexcept
{
    ...
    else
    {
        ...
        if(!r[index].isNull())
        {
            suggestionWinnerId_=std::make_shared<uint32_t>(r[index].as<uint32_t>());
        }
        index = offset + 3;
        if(!r[index].isNull())
        {
...
Sprints::Sprints(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[2].empty() && pJson.isMember(pMasqueradingVector[2]))
    {
        ...
        {
            suggestionWinnerId_=std::make_shared<uint32_t>((uint32_t)pJson[pMasqueradingVector[2]].asUInt64());
        }
    }
...
void Sprints::updateByMasqueradedJson(const Json::Value &pJson,
...
const std::shared_ptr<uint32_t> &Sprints::getId() const noexcept
{
    return id_;
}
...
void Sprints::setSuggestionWinnerId(const uint32_t &pSuggestionWinnerId) noexcept
{
...
bool Sprints::validateJsonForCreation(const Json::Value &pJson, std::string &err)
{
    if(pJson.isMember("id"))
    {
...
bool Sprints::validateMasqueradedJsonForCreation(const Json::Value &pJson,
...
bool Sprints::validateMasqueradedJsonForUpdate(const Json::Value &pJson,
                                               const std::vector<std::string> &pMasqueradingVector,
                                               std::string &err)
{
    if(pMasqueradingVector.size() != 5)
...
bool Sprints::validJsonOfField(size_t index,
